...
async function populateTranslationDropdown() {
    ...
    // Merge custom translations from storage
    const items = await new Promise(resolve => {
        chrome.storage.sync.get({ [STORAGE_KEY_CUSTOM_TRANSLATIONS]: {} }, resolve);
    });
    const customTrans = items[STORAGE_KEY_CUSTOM_TRANSLATIONS] || {};
    Object.assign(bibles, customTrans);
    BIBLE_DATA_CACHE = bibles;
...
async function handleGoClick() {
    ...
    try {
        if (apiType === 'esv.org' || apiType === 'esv_org') {
            ...
            // OSIS book lookup (simplified — matches content.js osisBookMap)
            const osisLookup = {
            ...
            const osisBook = osisLookup[canonicalBookName];
            if (!osisBook) throw new Error(`OSIS code not found for: ${canonicalBookName}`);
            let passageId = `${osisBook}.${chapter}`;
            if (!isFullChapter) passageId += `.${startVerse}`;
            if (endVerseStr) passageId += `-${osisBook}.${chapter}.${endVerseStr}`;
            const url = `https://api.scripture.api.bible/v1/bibles/${bibleId}/passages/${encodeURIComponent(passageId)}?content-type=html&include-notes=false&include-titles=true&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=true`;
            const response = await fetchWithTimeout(url, { headers: { 'api-key': apiKeyToUse } });
            if (!response.ok) throw new Error(`API.Bible ${response.status} ${response.statusText}`);
            const data = await response.json();
            fetchedHtml = data.data && data.data.content
                ? data.data.content.trim()
                : '<em class="error-message">Passage not found (API.Bible).</em>';
